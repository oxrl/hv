...
if(!defined('ABSPATH')){
  die(__('You are not allowed to call this page directly.', 'wp-user-avatar'));
...
function wpua_do_before_avatar(){ ?>
  ...
  <?php else : // Add to profile with admin style ?>
    <h3><?php _e('Avatar','wp-user-avatar') ?></h3>
...
if(!class_exists('wp_user_avatar')){
  class wp_user_avatar{
    ...
    function wpua_action_show_user_profile($user){
      ...
      $profile = ($current_user->ID == $user->ID) ? '&ldquo;'.__('Update Profile', 'wp-user-avatar').'&rdquo;' : '&ldquo;'.__('Update User', 'wp-user-avatar').'&rdquo;';
    ?>
      ...
      <?php if(current_user_can('upload_files')) : // Button to launch Media uploader ?>
        <p id="wpua-add-button"><button type="button" class="button" id="wpua-add" name="wpua-add"><?php _e('Edit Image','wp-user-avatar'); ?></button></p>
      <?php elseif(!current_user_can('upload_files') && !has_wp_user_avatar($current_user->ID)) : // Upload button ?>
        <p id="wpua-upload-button">
          ...
          <button type="submit" class="button" id="wpua-upload" name="submit" value="<?php _e('Upload','wp-user-avatar'); ?>"><?php _e('Upload','wp-user-avatar'); ?></button>
        </p>
        <p id="wpua-upload-messages">
          <span id="wpua-max-upload"><?php printf(__('Maximum upload file size: %d%s.', 'wp-user-avatar'), esc_html($wpua_upload_size_limit_with_units), esc_html('KB')); ?></span>
          <span id="wpua-allowed-files"><?php _e('Allowed Files','wp-user-avatar'); ?>: <?php _e('<code>jpg jpeg png gif</code>','wp-user-avatar'); ?></span>
        </p>
      <?php elseif((bool) $wpua_edit_avatar == 1 && !current_user_can('upload_files') && has_wp_user_avatar($current_user->ID) && wpua_author($wpua, $current_user->ID)) : // Edit button ?>
        ...
        <p id="wpua-edit-button"><button type="button" class="button" id="wpua-edit" name="wpua-edit" onclick="window.open('<?php echo $edit_attachment_link; ?>', '_self');"><?php _e('Edit Image','wp-user-avatar'); ?></button></p>
      <?php endif; ?>
      <p id="wpua-preview">
        <img src="<?php echo $avatar_medium; ?>" alt="" />
        <?php _e('Original Size','wp-user-avatar'); ?>
      </p>
      <p id="wpua-thumbnail">
        <img src="<?php echo $avatar_thumbnail; ?>" alt="" />
        <?php _e('Thumbnail','wp-user-avatar'); ?>
      </p>
      <p id="wpua-remove-button"><button type="button" class="button<?php echo $hide_remove; ?>" id="wpua-remove" name="wpua-remove"><?php _e('Remove','wp-user-avatar'); ?></button></p>
    ...
    function wpua_handle_upload_prefilter($file){
      ...
      if($size > $wpua_upload_size_limit){
        wp_die(__('The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.', 'wp-user-avatar'));
      }
      return $file;
    }
        ...
        if(isset($_POST['submit']) && $_POST['submit'] && !empty($_FILES['wpua-file'])){
          ...
          if(!empty($_FILES['wpua-file']['type'])){
            ...
            if(!preg_match('/(jpe?g|gif|png)$/i', $type)){
              wp_die(__('Sorry, this file type is not permitted for security reasons.', 'wp-user-avatar'));
            } else {
              // Break out file info
    ...
    function wpua_add_attachment_field_to_edit($fields, $post){
      ...
      $button = '<button type="button" class="button" id="set-wp-user-avatar-image" name="set-wp-user-avatar-image" onclick="wpuaSetAvatar(\''.$post->ID.'\', \''.$image[0].'\')">'.__('Select Image', 'wp-user-avatar').'</button>';
    ...
    function wpua_plugin_settings_links($links, $file){
      if(basename($file) == basename(plugin_basename(__FILE__))){
        $settings_link = '<a href="'.add_query_arg(array('page' => 'wp-user-avatar'), admin_url('options-general.php')).'">'.__('Settings', 'wp-user-avatar').'</a>';
    ...
    function wpua_media_upload_scripts($user=""){
      ...
      if($pagenow == 'options-discussion.php' || ($pagenow == 'options-general.php' && isset($_GET['page']) && $_GET['page'] == 'wp-user-avatar')){
        ...
        $wpua_custom_scripts = array('section' => __('Default Avatar', 'wp-user-avatar'), 'edit_image' => __('Edit Image', 'wp-user-avatar'), 'select_image' => __('Select Image', 'wp-user-avatar'), 'avatar_thumb' => $mustache_admin);
        ...
        $avatar_medium_src = (bool) $show_avatars == 1 ? wpua_get_avatar_original($user->user_email, 96) : includes_url().'images/blank.gif';
        $wpua_custom_scripts = array('section' => $user->display_name, 'edit_image' => __('Edit Image', 'wp-user-avatar'), 'select_image' => __('Select Image', 'wp-user-avatar'), 'avatar_thumb' => $avatar_medium_src);
        wp_localize_script('wp-user-avatar', 'wpua_custom', $wpua_custom_scripts);
      }
    }
  }
  ...
  function wpua_add_default_avatar($avatar_list=null){
    ...
    $wpua_list .= '<p id="wpua-edit"><button type="button" class="button" id="wpua-add" name="wpua-add">'.__('Edit Image', 'wp-user-avatar').'</button>';
    $wpua_list .= '<a href="#" id="wpua-remove"'.$hide_remove.'>'.__('Remove', 'wp-user-avatar').'</a></p>';
    $wpua_list .= '<input type="hidden" id="wp-user-avatar" name="avatar_default_wp_user_avatar" value="'.$wpua_avatar_default.'">';
    $wpua_list .= '<p id="wpua-message">'.sprintf(__('Click %s to save your changes', 'wp-user-avatar'), '&ldquo;'.__('Save Changes', 'wp-user-avatar').'&rdquo;').'</p>';
    if((bool) $wpua_disable_gravatar != 1){
  ...
  function wpua_add_media_state($media_states){
    ...
    if(!empty($is_wpua)){
      $media_states[] = __('Avatar', 'wp-user-avatar');
    }
    if(!empty($wpua_avatar_default) && ($wpua_avatar_default == $post->ID)){
      $media_states[] = __('Default Avatar', 'wp-user-avatar');
    }
  ...
  ?>
    <div class="wrap">
      ...
      <form method="post" action="options.php">
        ...
        <table class="form-table">
          <tr valign="top">
            <th scope="row"><?php _e('Settings','wp-user-avatar'); ?></th>
            <td>
              <fieldset>
                <legend class="screen-reader-text"><span><?php _e('Settings','wp-user-avatar'); ?></span></legend>
            ...
            <tr valign="top">
              ...
              <td>
                <fieldset>
                  ...
                  <span id="wpua-readable-size-error"><?php printf(__('%s exceeds the maximum upload size for this site.', 'wp-user-avatar'), ""); ?></span>
                  <div id="wpua-slider"></div>
                  <span class="description"><?php printf(__('Maximum upload file size: %d%s.', 'wp-user-avatar'), esc_html(wp_max_upload_size()), esc_html(' bytes ('.$upload_size_limit_with_units.')')); ?></span>
                </fieldset>
                <fieldset>
                ...
                <fieldset id="wpua-resize-sizes"<?php echo $hide_resize; ?>
                  <br />
                  <br />
                  <label for="wp_user_avatar_resize_w"><?php _e('Width','wp-user-avatar'); ?></label>
                  ...
                  <label for="wp_user_avatar_resize_h"><?php _e('Height','wp-user-avatar'); ?></label>
        ...
        <h3 class="title"><?php _e('Avatars','wp-user-avatar'); ?></h3>
        <p><?php _e('An avatar is an image that follows you from weblog to weblog appearing beside your name when you comment on avatar enabled sites. Here you can enable the display of avatars for people who comment on your site.','wp-user-avatar'); ?></p>
        <table class="form-table">
          <tr valign="top">
