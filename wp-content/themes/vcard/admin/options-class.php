...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					if (isset($vcard_data[$value['id']])) {
							$t_value = stripslashes($vcard_data[$value['id']]);
						} else {
							$t_value = '';
						}
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {					
						if (isset($vcard_data[$value['id']])) {
							$vcard_data_output = $vcard_data[$value['id']];
						} else {
							$vcard_data_output = ''; 
						}
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected($vcard_data_output, $option, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($vcard_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				...
				case 'checkbox':
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($vcard_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					$multi_stored = $vcard_data[$value['id']];
				...
				case 'color':		
					if (isset($vcard_data[$value['id']])) {
						$vcard_data_output = $vcard_data[$value['id']];
					} else { $vcard_data_output = ''; }
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$vcard_data_output.'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'.$vcard_data_output.'" />';
				break;
				
				//typography option	
				case 'typography':
				
					$typography_stored = isset($vcard_data[$value['id']]) ? $vcard_data[$value['id']] : $value['std'];
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $vcard_data[$value['id']];
				...
				case 'images':
					...
					if (empty($vcard_data[$value['id']])) {
						$select_value = '';
					} else {
						$select_value = $vcard_data[$value['id']];
					}
				...
				case 'sorter':
					...
					$sortlists = isset($vcard_data[$value['id']]) && !empty($vcard_data[$value['id']]) ? $vcard_data[$value['id']] : $value['std'];
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($vcard_data[$value['id']]) && !empty($vcard_data[$value['id']]) ? $vcard_data[$value['id']] : '';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','vcard').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($vcard_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
	...
	public static function optionsframework_uploader_function($id,$std,$mod){
		...
		$uploader = '';
	    $upload = (isset($vcard_data[$id])) ? $vcard_data[$id] : '';
		$hide = '';
		...
		if(!empty($upload) && !($id == 'custom_font_woff' || $id == 'custom_font_ttf' || $id == 'custom_font_svg' || $id == 'custom_font_eot')){
	    	...
	    	$uploader .= '<img class="of-option-image" id="image_'.$id.'" src="'.$upload.'" alt="<?php the_title(); ?>" />';
	    	$uploader .= '</a>';
			$uploader .= '</div>';
			}
		$uploader .= '<div class="clear"></div>' . "\n"; 
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		if(!empty($val['url'])){
	    	...
	    	$slider .= '<img class="of-option-image" id="image_'.$id.'_'.$order .'" src="'.$val['url'].'" alt="<?php the_title(); ?>" />';
	    	$slider .= '</a>';
