...
function optionsframework_options_page(){
	...
	*/	
	...
	include_once( ADMIN_PATH . 'main-options.php' );
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$vcard_data = get_option(BACKUPS);
		
		update_option(OPTIONS, $vcard_data);
		
		die('1'); 
	}
	elseif($save_type == 'import_options'){
			
		$vcard_data = $_POST['data'];
		$vcard_data = unserialize(base64_decode($vcard_data)); //100% safe - ignore theme check nag
		update_option(OPTIONS, $vcard_data);
	...
	{
		wp_parse_str(stripslashes($_POST['data']), $vcard_data);
		unset($vcard_data['security']);
		unset($vcard_data['of_save']);
		update_option(OPTIONS, $vcard_data);
